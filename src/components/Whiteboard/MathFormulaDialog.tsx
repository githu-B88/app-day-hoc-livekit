...
import { MATH_FORMULAS } from '../../data/stemTools';
...
export const MathFormulaDialog: React.FC<MathFormulaDialogProps> = ({
  ...
  const [latexInput, setLatexInput] = useState<string>('\\int_{0}^{\\pi} \\sin(x)\\,dx = 2');
  const [selectedFormulaTitle, setSelectedFormulaTitle] = useState<string>('Tích phân lượng giác');
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/60">
          <div className="flex items-center gap-2.5 text-emerald-400">
            ...
            <div>
              <h3 className="font-semibold text-slate-100 text-base">Chèn Công Thức Toán Học (LaTeX / KaTeX)</h3>
              <p className="text-xs text-slate-400">Hiển thị sắc nét 1080p trực tiếp trên bảng xanh</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-slate-300 flex items-center gap-1.5">
                ...
                Mẫu công thức phổ biến (Toán 10 - 11 - 12):
              </label>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Nhập mã LaTeX, ví dụ: \lim_{x \to 0} \frac{\sin x}{x} = 1"
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] text-slate-400 font-mono">Độ phân giải cao</span>
            </div>
